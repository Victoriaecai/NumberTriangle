...
public class NumberTriangle {
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        List<NumberTriangle> prevRow = new ArrayList<>(); //stores previous row later
        ...
        while (line != null) {
            //make new array of strings for the nodes in this line
            String[] parts = line.split(" ");
            // remove when done; this line is included so running starter code prints the contents of the file
            List<NumberTriangle> currRow = new ArrayList<>();

            // adds the row's numbers to a new NumberTriangle object in curr row
            ...
            if (!prevRow.isEmpty()) {
                for (int i = 0; i < prevRow.size(); i++) {
                    //adds left and right to the respective parent nodes from prevRow
                    prevRow.get(i).setLeft(currRow.get(i));
                    prevRow.get(i).setRight(currRow.get(i + 1));
                }
            } else { // curr is top row
                top = currRow.get(0);
            }
